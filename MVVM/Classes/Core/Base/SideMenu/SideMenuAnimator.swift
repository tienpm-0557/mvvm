...
public extension SideMenu {
    struct ContentViewShadow {
        ...
        public init(enabled: Bool = true, color: UIColor = UIColor.black, offset: CGSize = CGSize.zero, opacity: Float = 0.4, radius: Float = 8.0) {
    ...
    struct MenuViewEffect {
        ...
        public init(fade: Bool = true, scale: Bool = true,
    ...
    struct ContentViewEffect {
        ...
        var landscapeOffsetX: Float = (UIDevice.current.userInterfaceIdiom == .pad) ? Float(400 - max(UIScreen.main.bounds.size.width,UIScreen.main.bounds.size.height)/2) : 100
        var portraitOffsetX: Float = (UIDevice.current.userInterfaceIdiom == .pad) ? Float(400 - min(UIScreen.main.bounds.size.width,UIScreen.main.bounds.size.height)/2) : 100
        ...
        public init(alpha: Float = 1.0,
             scale: Float = 0.7,
             landscapeOffsetX: Float = (UIDevice.current.userInterfaceIdiom == .pad) ? Float(400 - max(UIScreen.main.bounds.size.width,UIScreen.main.bounds.size.height)/2) : 30,
             portraitOffsetX: Float = (UIDevice.current.userInterfaceIdiom == .pad) ? Float(min(UIScreen.main.bounds.size.width,UIScreen.main.bounds.size.height)/2 - 400) : Float(UIScreen.main.bounds.size.width / 2 - 100),
             minParallaxContentRelativeValue: Float = -25.0, maxParallaxContentRelativeValue: Float = 25.0,
