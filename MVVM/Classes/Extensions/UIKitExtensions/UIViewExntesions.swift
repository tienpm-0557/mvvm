...
public extension UIView {
    ...
    func getGesture<G: UIGestureRecognizer>(_ comparison: ((G) -> Bool)? = nil) -> G? {
        return gestureRecognizers?.first(where: { gesture -> Bool in
            if let comparison = comparison, let ges = gesture as? G {
...
public extension UIView {
    ...
    func setupBackgroundGradient(colorTop: CGColor, colorBottom: CGColor) {
        ...
        gradl.colors = [colorTop, colorBottom]
        gradl.startPoint = CGPoint(x: 0, y: 0.5)
        gradl.endPoint = CGPoint(x: 1, y: 0.5)
        gradl.frame = self.bounds
        self.layer.insertSublayer(gradl, at: 0)
    }
}
